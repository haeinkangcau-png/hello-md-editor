...
export function mdInline(s) {
  ...
  return s
    ...
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (m, text, href) =>
      href.charAt(0) === '#'
        // 앵커 링크: 제목 id 규칙(makeHeadingId)에 맞춰 문서 내 이동용으로 렌더
        ? `<a href="#${makeHeadingId(href.slice(1))}" class="dp-link dp-anchor">${text}</a>`
        : `<a href="${href}" title="${href}" target="_blank" rel="noopener" class="dp-link">${text}</a>`)
